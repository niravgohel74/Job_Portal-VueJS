      ...
      <section class="bg-white px-10 py-10">
        <div class="container-xl lg:container m-auto">
          ...
          <div v-else class="grid grid-cols-1 gap-10 md:grid-cols-3">
            <JobListing v-for="job in state.jobs.slice(0, 6 || state.jobs.length)" :key="job.id" :job="job"
              class="border border-gray-300 hover:shadow-2xl" />
            <JobListing v-for="job in state.jobs.slice(7, limit || state.jobs.length)" :key="job.id" :job="job"
              class="border border-gray-300 hover:shadow-2xl" />
          </div>
